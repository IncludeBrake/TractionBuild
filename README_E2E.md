...
docker exec -it zerotoship-postgres-1 psql -U postgres -d zerotoship -f /docker-entrypoint-initdb.d/init_db.sql
```
...
### Core Components
- **Schemas** (`src/zerotoship/schemas/core.py`) - Frozen contracts for all crews
- **Adapters** (`src/zerotoship/crews/adapters.py`) - Wrap existing crews with schema enforcement
- **Workflow Engine** (`src/zerotoship/core/workflow_engine.py`) - Drives Validator → Advisory → COMPLETED
- **API** (`src/zerotoship/api/app.py`) - Clean REST API with 3 endpoints
...
### Metrics Available
- `zerotoship_requests_total` - Total API requests
- `zerotoship_requests_success` - Successful requests
- `zerotoship_requests_error` - Failed requests
- `zerotoship_request_duration_seconds` - Average request latency
- `zerotoship_projects_created` - Total projects created
- `zerotoship_projects_retrieved` - Total projects retrieved
...
2. Create adapter in `src/zerotoship/crews/adapters.py`
...
```bash
# View API logs
docker logs zerotoship-api-1

# View database logs
docker logs zerotoship-postgres-1
docker logs zerotoship-neo4j-1
```
