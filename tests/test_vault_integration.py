#!/usr/bin/env python3
"""
Test script for Vault integration with LLM configuration.
...
from tractionbuild.utils.llm_config import LLMConfig, llm_config
...
def test_vault_client():
    ...
    # Test Anthropic secrets
    anthropic_secrets = vault.read_llm_secrets("anthropic")
    if anthropic_secrets:
        print(f"✅ Anthropic secrets found: {list(anthropic_secrets.keys())}")
    else:
        print("⚠️  No Anthropic secrets found in Vault")
...
def test_llm_config():
    ...
    print("\n🔧 Testing Anthropic LLM Configuration...")
    anthropic_llm = config.get_anthropic_llm()
    if anthropic_llm:
        print("✅ Anthropic LLM configured successfully")
        print(f"   Model: {anthropic_llm.model}")
    else:
        print("❌ Anthropic LLM configuration failed")
...
def test_connection():
    ...
    openai_test = config.test_llm_connection("openai")
    print(f"OpenAI Test Results: {openai_test}")
    
    # Test Ollama connection
    print("\n🔗 Testing Ollama Connection...")
    ollama_test = config.test_llm_connection("ollama")
    print(f"Ollama Test Results: {ollama_test}")
...
def main():
    ...
    try:
        ...
        # Test connections (optional - may fail if no API keys)
        print("\n" + "=" * 50)
        print("🔗 Connection tests (may fail if no API keys configured):")
        test_connection()
