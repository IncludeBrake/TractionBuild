...
if PROMETHEUS_AVAILABLE:
    REQUEST_TIME = Summary('zerotoship_request_processing_seconds', 'Time spent processing request')
    WORKFLOW_EXECUTIONS = Counter('zerotoship_workflow_executions_total', 'Total workflow executions', ['workflow_name', 'status'])
    CREW_EXECUTIONS = Counter('zerotoship_crew_executions_total', 'Total crew executions', ['crew_name', 'status'])
...
class ZeroToShipOrchestrator:
    """Production-ready orchestrator for ZeroToShip workflows."""
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            logger.info("ZeroToShip orchestrator initialized successfully")
            return self
        except Exception as e:
...
async def run_workflow(idea: str, workflow_name: str):
    ...
    project_id = f"project_{uuid.uuid4().hex[:8]}"
    print(f"🚀 Starting ZeroToShip for idea: '{idea}'")
...
async def main(idea: str, workflow_name: str = "default_software_build"):
    ...
    logger.info(f"🚀 Starting ZeroToShip for '{idea}' with workflow '{workflow_name}'")
    ...
    try:
        ...
        logger.error(f"ZeroToShip execution failed: {e}")
        if PROMETHEUS_AVAILABLE:
