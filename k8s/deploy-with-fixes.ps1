...
# Securely prompt for API keys if not provided (avoid plaintext in params for production)
if (-not $OpenAIAPIKey) {
    $OpenAIAPIKey = Read-Host "Enter OpenAI API Key (will be injected to Vault)"
}
if (-not $AnthropicAPIKey) {
    $AnthropicAPIKey = Read-Host "Enter Anthropic API Key (will be injected to Vault)"
}

# Function for error handling and rollback
...
# Get kubectl version for compatibility
$kubectlVersion = kubectl version --client --output=json | ConvertFrom-Json
$major = $kubectlVersion.clientVersion.major
$minor = $kubectlVersion.clientVersion.minor
...
kubectl get deployment zerotoship-app -o yaml > k8s/zerotoship-deployment-backup.yaml

# Scale down with version check
Write-Host "Scaling down deployment to clean up..." -ForegroundColor Cyan
try {
    if ($major -ge 1 -and $minor -ge 18) {
        kubectl scale deployment zerotoship-app --replicas=0 --ignore-not-found=true -n $Namespace
    } else {
        kubectl scale deployment zerotoship-app --replicas=0 -n $Namespace 2>$null
    }
} catch {
    Handle-Error "Failed to scale down deployment" "kubectl apply -f k8s/zerotoship-deployment-backup.yaml"
}
...
do {
    ...
    if ($elapsed -ge $timeout) { Handle-Error "Timeout waiting for pods to terminate" }
...
do {
    ...
    if ($elapsed -ge $timeout) { Handle-Error "Timeout binding PVCs" "kubectl delete -f k8s/zerotoship-deployment.yaml" }
